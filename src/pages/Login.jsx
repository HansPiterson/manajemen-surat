...
export default function Login() {
  ...
  const handleRegister = async (e) => {
    ...
    // Formulate division-based email format
    const emailToUse = `${kodeDivisi.toLowerCase()}_${nama.toLowerCase().replace(/\s+/g, '')}@pttimah.com`;
    ...
    try {
      ...
      if (!signUpData.user) throw new Error("Registrasi gagal dilakukan.");

      // 2. Insert record to public.users table to link divisions
        ...
        .insert([
          {
            ...
            email: emailToUse,
            role: 'divisi',
            divisi_id: selectedDivisiId || null,
  ...
  const handleLogin = async (e) => {
    ...
    // Example: Kode="IT", Nama="Support" -> it_support@pttimah.com
    const emailToUse = isDivisionLogin 
      ? `${kodeDivisi.toLowerCase()}_${nama.toLowerCase().replace(/\s+/g, '')}@pttimah.com`
      : email;
        ...
        <div className="p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm">
              {isRegister ? 'Daftar akun divisi baru' : 'Masuk ke sistem manajemen pengiriman'}
            </p>
          </div>
          ...
          <form onSubmit={isRegister ? handleRegister : handleLogin} className="space-y-5">
            {!isDivisionLogin ? (
              ...
              <div className="space-y-4">
                {isRegister && (
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5" htmlFor="namaLengkap">
                      Nama Lengkap *
                    </label>
                    <div className="relative">
                      <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400">
                      ...
                      <input
                        ...
                        className="block w-full pl-10 pr-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-900/50 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all sm:text-sm"
                        placeholder="Nama Lengkap Divisi / Staff"
                        required={isRegister}
                      />
                    </div>
                  </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5" htmlFor="kodeDivisi">
                    Kode Divisi * (untuk email, e.g. IT)
                  </label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400">
                ...
                {isRegister && (
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5" htmlFor="selectDivisi">
                      Pilihan Divisi Terdaftar *
                    </label>
                    <select
                      id="selectDivisi"
                      ...
                      required={isRegister}
                      className="block w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-900/50 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all sm:text-sm"
                    >
                      <option value="">-- Hubungkan ke Divisi Terdaftar --</option>
                      {divisiList.map(div => (
                        <option key={div.id} value={div.id}>{div.nama_divisi} ({div.kode_divisi})</option>
                      ))}
                    </select>
                  </div>
                )}
            ...
            {isRegister && (
              <div className="text-xs text-slate-500 dark:text-slate-400">
                Email Anda akan terdaftar sebagai:{' '}
                <span className="font-semibold text-slate-700 dark:text-slate-200">
                  {kodeDivisi ? kodeDivisi.toLowerCase() : '[kode]'}_{nama ? nama.toLowerCase().replace(/\s+/g, '') : '[nama]'}@pttimah.com
                </span>
              </div>
            )}
